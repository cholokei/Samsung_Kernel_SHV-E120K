...
void mhl_hpd_handler(bool state)
{
	...
	if(mhl_hpd_state == false && state == false) {
		DEV_INFO("mhl_hpd_handler - already off, do nothing..\n");
		return;
	}
	if (state == false) {
...
#endif
			/* audio framework takes delay in WiredAccessoryObserver - jgk.20120623 */
			msleep(200);
...
static irqreturn_t hdmi_msm_isr(int irq, void *dev_id)
{
	...
	if ((hdcp_int_val & (1 << 6)) && (hdcp_int_val & (1 << 4))) {
		...
		if (hdmi_msm_state->full_auth_done && mhl_hpd_state) {
			...
			mutex_unlock(&hdcp_auth_state_mutex);
		if( (link0_status_val & (1<<4)) &&
			(link0_status_val & (1<<5)) &&
